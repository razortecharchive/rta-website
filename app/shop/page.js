...
const products = [
  { id: 'razor', name: 'RTA Original Razor', price: 18000, tag: 'Tool', desc: 'Stealth cut のために設計されたオリジナルレザー。静寂と精度を支える一本。' },
  { id: 'tote', name: 'RTA Tote Bag', price: 5500, tag: 'Apparel', desc: 'ナチュラルキャンバスに型押しのロゴ。日々の道具を運ぶための、控えめな相棒。' },
  { id: 'zine', name: 'RTA Zine Vol.01', price: 3300, tag: 'Publication', desc: '理論と感覚を綴じた、最初のアーカイブ印刷物。' },
];
    ...
    <main style={{background:'#EDEBE5', minHeight:'100vh', fontFamily:'DM Sans, sans-serif', fontWeight:200, color:'#1C1A17'}}>
      ...
      <section className="shop-header" style={{padding:'180px 80px 80px', borderBottom:'1px solid #C4BFB7'}}>
        ...
        <h1 className="shop-heading" style={{fontFamily:'Cormorant Garamond, serif', fontSize:'clamp(48px,7vw,108px)', fontWeight:200, lineHeight:0.95, letterSpacing:'-0.02em', marginBottom:36}}>
          Razor Tech<br/><em style={{fontStyle:'italic', color:'#C9956A'}}>Goods</em>
        </h1>
        <p style={{fontFamily:"'Hiragino Mincho Pro', 'ヒラギノ明朝 Pro', serif", fontSize:13, color:'#9A948C', lineHeight:2.1, maxWidth:540}}>道具と思想を、形に。Stealth cut の理論を支えるために、選び抜かれた最小限のプロダクト。</p>
      </section>
          ...
          {products.map((p, idx) => (
            <article key={p.id} className="product-card" style={{display:'flex', flexDirection:'column'}}>
              <div style={{aspectRatio:'3/4', background:'#E5E2DA', border:'1px solid #C4BFB7', marginBottom:28, position:'relative', overflow:'hidden'}}>
                ...
                <span style={{position:'absolute', top:16, right:16, fontSize:8, letterSpacing:'0.3em', textTransform:'uppercase', color:'#C9956A'}}>{p.tag}</span>
                <span style={{position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', fontFamily:'Cormorant Garamond, serif', fontSize:10, letterSpacing:'0.3em', color:'#C4BFB7', textTransform:'uppercase'}}>Product Image</span>
              </div>
              ...
              <div style={{marginTop:'auto', paddingTop:24, borderTop:'1px solid #C4BFB7', display:'flex', alignItems:'center', justifyContent:'space-between', gap:16, flexWrap:'wrap'}}>
                ...
                <button
                  ...
                  style={{
                    ...
                    color: loadingId === p.id ? '#9A948C' : '#1C1A17',
                ...
                >
                  {loadingId === p.id ? 'Loading…' : 'Purchase →'}
                </button>
              </div>
            </article>
          ))}
