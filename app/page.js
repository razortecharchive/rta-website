...
export default function Home() {
  ...
  return (
    <main style={{background:'#EDEBE5', minHeight:'100vh', fontFamily:'DM Sans, sans-serif', fontWeight:200, color:'#1C1A17'}}>
      ...
      {/* NAV */}
      <nav style={{position:'fixed', top:0, left:0, right:0, zIndex:200, padding:'26px 48px', display:'flex', alignItems:'center', justifyContent:'space-between', background:'rgba(237,235,229,0.9)', backdropFilter:'blur(12px)', borderBottom:'1px solid #C4BFB7'}}>
      ...
      {/* MENU OVERLAY */}
      {menuOpen && (
        <div style={{position:'fixed', inset:0, background:'#E5E2DA', zIndex:150, display:'flex', flexDirection:'column', justifyContent:'center', padding:'120px 80px 80px'}}>
          ...
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-end'}}>
            <p style={{fontFamily:'Cormorant Garamond, serif', fontSize:12, fontStyle:'italic', color:'#9A948C', lineHeight:1.8}}>Precision has no language.<br/>感覚を理論へ。</p>
      ...
      <section style={{position:'relative', width:'100vw', height:'100vh', background:'#EDEBE5', overflow:'hidden'}}>
        ...
        <div style={{position:'absolute', bottom:40, left:0, right:0, padding:'0 48px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', zIndex:10}}>
      ...
      <div style={{borderTop:'1px solid #C4BFB7', borderBottom:'1px solid #C4BFB7', padding:'13px 0', overflow:'hidden'}}>
        <div style={{display:'flex', gap:60, animation:'tick 28s linear infinite', whiteSpace:'nowrap'}}>
          {['Razor Tech Archive','Stealth Cut','Precision Has No Language','感覚を理論へ','From Sensation to Theory','Razor Tech Archive','Stealth Cut','Precision Has No Language','感覚を理論へ','From Sensation to Theory'].map((t,i) => (
            <span key={i} style={{fontSize:8, letterSpacing:'0.35em', textTransform:'uppercase', color:'#9A948C', flexShrink:0}}>{t} &nbsp;✦&nbsp; </span>
          ))}
        </div>
      </div>

      {/* SEMINAR */}
      <section style={{padding:'140px 80px', borderTop:'1px solid #C4BFB7'}}>
        ...
        {[
          ...
          <a key={s.num} href="#" style={{display:'grid', gridTemplateColumns:'180px 1fr auto auto', alignItems:'center', gap:48, padding:'36px 0', borderBottom:'1px solid #C4BFB7', textDecoration:'none', color:'#1C1A17', opacity: s.dim ? 0.35 : 1}}>
            <span style={{fontSize:10, color:'#9A948C', letterSpacing:'0.12em'}}>{s.num} — {s.date}</span>
            <span style={{fontFamily:'Cormorant Garamond, serif', fontSize:'clamp(18px,2.4vw,30px)', fontWeight:300}}>{s.name}</span>
            <span style={{fontSize:10, color:'#9A948C', letterSpacing:'0.08em'}}>{s.loc}</span>
      ...
      <section style={{padding:'140px 80px', borderTop:'1px solid #C4BFB7'}}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:120}}>
          <div>
            <div style={{fontFamily:'Cormorant Garamond, serif', fontSize:'clamp(40px,5vw,72px)', fontWeight:200, lineHeight:0.95, letterSpacing:'-0.02em', marginBottom:48}}>
          ...
          <div style={{paddingTop:8}}>
            <p style={{fontSize:12, lineHeight:2.1, color:'#9A948C', marginBottom:24}}>Razor Tech Archive（RTA）は、レザーカット技術の教育・普及を専門とする教育プラットフォームです。</p>
            <p style={{fontSize:12, lineHeight:2.1, color:'#9A948C', marginBottom:24}}><strong style={{color:'#1C1A17', fontWeight:300}}>「stealth cut」</strong>——音もなく、摩擦もなく、ただ正確に。フクロウの飛翔のように静寂の中に宿る技術を、理論と感覚の両軸から追求する。</p>
            <p style={{fontSize:12, lineHeight:2.1, color:'#9A948C'}}>セミナー、アーカイブコンテンツ、技術者コミュニティを通じて、カット技術の新しい基準を提案します。</p>
          </div>
        </div>
      </section>
      ...
      {/* MEMBERS */}
      <section style={{padding:'140px 80px', borderTop:'1px solid #C4BFB7'}}>
        <div style={{border:'1px solid #C4BFB7', padding:'100px 80px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:60, position:'relative', overflow:'hidden'}}>
          <div>
            ...
            <h2 style={{fontFamily:'Cormorant Garamond, serif', fontSize:'clamp(32px,4vw,56px)', fontWeight:200, lineHeight:1.0, marginBottom:20, letterSpacing:'-0.01em'}}>Join the<br/>Archive</h2>
            <p style={{fontSize:12, color:'#9A948C', lineHeight:2, maxWidth:400}}>会員限定コンテンツ、オンラインセミナー、アプリへのアクセス。技術を深める場所がここにあります。</p>
          </div>
      ...
      {/* FOOTER */}
      <footer style={{padding:'48px 80px', borderTop:'1px solid #C4BFB7', display:'flex', alignItems:'center', justifyContent:'space-between'}}>
